...
public class UserTest {
	...
	public void testAggregationQuery() {
		...
		// e.g. some will have the same username, but will not be in the range etc. Does this have value? Are we just making tautology tests?
		
		// library ensures that edge cases are created. i.e. E.g. there will be a similar (exactly the same?) person born on 1990-01-01, to protect from off by one error
		...
		List<User> users = randomUserBuilder
				...
				.build(numberOfUsers); // creates 50 users based on randomUserBuilder and exactly 10 users based on exactUserBuilder where uniqeRules from exactUserBuilder
		// 
		...
		List<User> exactUsers = exactUserBuilder
				.randomUsernameFrom("destroyerOfW0rldz", "univerzalBlack", "johnycage", "subzero", "krelac", "pevac", "hardy")
				.randomBirthDateBetween(newOldestBirthDate, newYougnestBirthDate) // .exclusiveRule() // means that when combined with other builders no users built by other builders can satisfy this rules, i.e. only this builder can build users in that range of birthdates
				// rule - 
